var demo = {
	...
	resetGleye: function(gl3dview) {
		gl3dview.getGleye().setPosition(100, 900, 2300);
	...
	addshebei:function(gl3dview){
		...
		if(content) {
			...
			var html = html + "<tbody>"+
							  "</tbody>";
			content.innerHTML="<span class='btn addtr'>添加一行</span><br/><table class='shebei'><thead><th style='width:120px'>设备名称</th>"+
											"<th style='width:80px'>起始位置</th>"+
											"<th style='width:80px'>所占u</th>"+
											"<th>设备类型</th>"+
											"<th style='width:90px'>操作</th></thead>"+
											html+
											"</table>"
			div.appendChild(content);
		}
		...
		btns.style.left = '3px';
		btns.style.top = '414px';
		btns.style.width = (width - 6) + 'px';
		btns.style.height ='32px';
		btns.style.background = '#FFFFFF';
		btns.style.textAlign = 'center';
		btns.innerHTML="<span class='btn btntrue'>确定</span><span class='btn btnfalse'>取消</span>"
		div.appendChild(btns);
		$(".addtr").click(function(){
			var trhtml = "<tr>"+
							"<td><input type='text' style='width:110px;margin-left:5px;border:1px solid #95918D;height:20px'></td>"+
							"<td><input type='text' class='startposition' style='width:60px;margin-left:5px;border:1px solid #95918D;height:20px'></td>"+
							"<td><select class='allposition' style='width:60px;margin-left:5px;border:1px solid #95918D;height:20px'></select></td>"+
							"<td><select style='width:124px;margin-left:5px;border:1px solid #95918D;height:20px'><option>交换机</option>"+
							"<option>防火墙</option><option>路由器</option><option>认证网关</option><option>回溯分析</option><option>流量清洗</option>"+
							"<option>流量监测</option><option>机架服务器</option><option>移动终端</option><option>服务器</option><option>传输设备</option><option>网络布线</option>"+
							"</select></td>"+
							"<td><span class='btn btndeteal' onclick='detrleshebei(this)'>删除</span></td>"+
						 "</tr>"
			$(".shebei tbody").append(trhtml)
		});
		$(".btntrue").click(function(){
			$.ajax({
		        ...
		        success: function (data) {
		        	...
		        	var trlength = $(".shebei tbody tr").length;
		        	var racksjson = [];
		        	for(var j=0;j<dataJson.objects.length;j++){
		        		if(dataJson.objects[j].type=="racks"){
		        			racksjson.push(dataJson.objects[j]);
		        			
		        		}
		        	}
		        	
	        		var rackjson=[];
	        		for(var m=0;m<racksjson[0].numbers.length;m++){ 
	        			if(racksjson[0].numbers[m].cabinetid=="cabinetid2"){
	        				//for(var i=0;i<trlength;i++){//循环tr添加设备，根据所占u和设备类别得到nubner
	        				//	
	        				//}
	        				racksjson[0].numbers[m].cabinetcon.push({
	        					ID:"ca4f0324-0851-4b2d-a37a-cd75550d4290",nubner: "2",posi:"10", correct: "true"
	        				});
	        				/*var gl3dview = new mono.Gl3dview3D();*/
	        				demo.loadData(gl3dview);
	        				$("#dialog").remove();		        				
...
demo.registerCreator('rack', function(box, json) {
	...
	rack.setClient('dbl.func', function() {
		...
		span2.setAttribute('class', 'addshebei');
		span2.innerHTML = "添加设备";
		document.body.appendChild(span2);
		span2.onclick = function() {
			var gl3dview = new mono.Gl3dview3D();
			demo.addshebei(gl3dview);
...
var dataJson = {
	objects: [{
		...
		height: 180,//高
		depth: 80,//长
		translate: [500, 0, -450],//坐标
		imgs:'kong_font.png'//正面图片
	},{
		type: 'floor_box',//挂式空调
		rotate: [0, Math.PI / 180 * 90, 0],//旋转角度
		width: 30,//宽
	...
	},
	{
		...
		height: 100,//高
		depth: 100,//长
		translate: [-450, 0, 450],//坐标
		imgs:'peidian.png'//正面图片
	},{
		type: 'wall',//墙
		height: 200,//高
		translate: [-500, 0, -500],//坐标
		//墙的四个点（闭合坐标）
		data: [
			[-400, 0],
			[1400, 0],
		...
		type: 'tv',//电视
		translate: [890, 100, 0],//位置坐标
		rotate: [0, Math.PI / 180 * 270, 0],//旋转角度
	},{
		type: 'gleye',//摄像头
		translate: [-860, 200, 0],//坐标位置
		angle: -90,//旋转角度
		statevideo:'test.mp4'//展示视频
	},{
		type: 'gleye',//摄像头
		translate: [860, 200, 400],//坐标位置
		angle: 90,//旋转角度
		statevideo:'test2.mp4'//展示视频
	},{
		type: 'gleye',//摄像头
		translate: [700, 200, -460],//坐标位置
		...
		numbers: [
			//A0
			{"cabinetid":"cabinetid1",angle: 0,cabinetcon:[//机柜ID 旋转角度 存储设备集合
				//存储设备ID									图片编号               起始位置	  是否报警
